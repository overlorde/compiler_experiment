...
Token lexer_next(Lexer *lexer) {
    ...
    if (c == '\'') {
        ...
        tok.line = lexer->line;
        tok.start = lexer->current - 3;
        tok.length = 3;
        tok.data.int_value = (int)(unsigned char)value;
        return tok;
    }
    ...
    if (isalpha(c) || c == '_') {
        ...
        else if (tok.length == 6 && strncmp(tok.start, "number", 6) == 0)
            tok.kind = TOK_KW_NUMBER;
        else if (tok.length == 4 && strncmp(tok.start, "char", 4) == 0)
            tok.kind = TOK_KW_CHAR;
...
const char *token_kind_name(TokenKind kind) {
    switch (kind) {
        ...
        case TOK_KW_NUMBER: return "number";
