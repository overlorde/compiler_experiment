...
/* Is the current token the start of a type? (int | bool | number | char) */
static int check_type_start(Parser *p) {
    return check(p, TOK_KW_INT) || check(p, TOK_KW_BOOL)
        || check(p, TOK_KW_NUMBER) || check(p, TOK_KW_CHAR);
}
...
static ASTNode *parse_primary_inner(Parser *p) {
    ...
    if (check(p, TOK_IDENT)) {
        ...
        if (check(p, TOK_LPAREN)) {
            ...
            int capacity = 4;
            int count = 0;
            ASTNode **args = (ASTNode **)malloc(capacity * sizeof(ASTNode *));
            ...
            if (!check(p, TOK_RPAREN)) {
                ...
                while (check(p, TOK_COMMA)) {
                    ...
                    if (count >= capacity) {
                        capacity *= 2;
                        args = (ASTNode **)realloc(args, capacity * sizeof(ASTNode *));
                    }
...
static ASTNode *parse_block(Parser *p) {
    ...
    int capacity = 8;
    int count = 0;
    ASTNode **stmts = (ASTNode **)malloc(capacity * sizeof(ASTNode *));
    ...
    while (!check(p, TOK_RBRACE) && !check(p, TOK_EOF)) {
        ...
        if (count >= capacity) {
            capacity *= 2;
            stmts = (ASTNode **)realloc(stmts, capacity * sizeof(ASTNode *));
        }
        stmts[count++] = stmt;
    }
...
static ASTNode *parse_function(Parser *p) {
    ...
    int param_cap = 4;
    int param_count = 0;
    Param *params = (Param *)malloc(param_cap * sizeof(Param));
    ...
    if (check_type_start(p)) {
        ...
        while (check(p, TOK_COMMA)) {
            ...
            if (param_count >= param_cap) {
                param_cap *= 2;
                params = (Param *)realloc(params, param_cap * sizeof(Param));
            }
            pname = (char *)malloc(p->current.length + 1);
            memcpy(pname, p->current.start, p->current.length);
            pname[p->current.length] = '\0';
            advance(p);
...
ASTNode *parser_parse(Parser *parser) {
    ...
    while (!check(parser, TOK_EOF)) {
        ...
        if (count >= capacity) {
            capacity *= 2;
            functions = (ASTNode **)realloc(functions, capacity * sizeof(ASTNode *));
        }
