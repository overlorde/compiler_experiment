...
- **Real language subset** — functions with parameters, `int`/`bool`/`number` types, arithmetic/comparison/logical operators, `if`/`while`/`for`, nested blocks, returns, and calls.
...
- **Subtyping via subsumption** — a single subtype relation (`Int <: Number`) is threaded through every premise via one `expect` helper, so every typing rule respects subsumption automatically with no per-rule changes.
